...
from settings import logger, NG_BASE
...
def process(triple_files, format="nt", dry=False, delete=False, sync=False, sleep=10, size=DEFAULT_BATCH_SIZE):
    ...
    for fpath in triple_files:
        ...
        if dry is True:
            ...
            if sync is True:
                ...
                logger.info("Deleting {} graph from {}.".format(fpath, named_graph))
                added = 0
                removed = vstore.bulk_remove(named_graph, g, size=size)
            else:
                logger.info("Adding {} graph to {}.".format(fpath, named_graph))
...
def verify(paths):
    ...
    if (args.delete is True) and (args.sync is True):
        raise Exception("Invalid action: --delete and --sync cannot be used together. Choose one.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Load triples')
    parser.add_argument('--dry-run', action="store_true", dest="dry", default=False, help="Dry run.")
    parser.add_argument('--delete', '-d', action="store_true", default=False, help="Delete this triple file.")
    ...
    verify(args.path)
    done = process(args.path, format=args.format, dry=args.dry, delete=args.delete, sync=args.sync, sleep=args.sleep, size=args.batch)
